...
| 26 | `s26` | TBD | **queued** — S27 fills the SHA in |
...
# S26 — the amplitudes derived and seeded, the arm number from the swing window (Phase 1b). SHA filled in by S27.
git tag -a s26 "$(git rev-list -1 --grep='^Merge S26 into main' main)" -m "S26: the amplitudes derived and seeded, the arm number from the disc"
