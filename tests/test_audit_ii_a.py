"""Audit II, aim (a) — every prediction the register and spec.MISSES made since S13, run (S21, branch
...
the repo unchanged (D114); nothing here fixes anything. The probes that ran on the simple model's
merger-split thick disc (#50, #19's cut-only law, row 7's constant) went with that model at D170;
the shared physics they also read is kept.
...
def test_the_merger_heating_constants_citation_read_moves_row_3_and_row_6(basic):
    ...
    27.06 km/s that is a constant of 112.3, which reads row 3 at 250.97 (in) and row 6 at 368 (out). (It read
