...
S9 gave the advanced model - the one model, ``basic``, since D170 - outflows, a
delay-time distribution and a chemical thin/thick split. This file re-runs the
constants and the predictions that were calibrated or made before those
mechanisms existed, and pins what it found. The simple model's own findings (its
merger-based thick disc, ``NET_YIELD``) went with it at D170.
...
def test_debt_17_the_zero_width_rows_say_no_testable_target():
    ...
    zero_width = (20, 21, 25, 26, 27, 28, 36)  # 36 joined at S35: the PNLF cutoff, no [O III] per nebula yet (D184)
    for n in zero_width:
    ...
    assert 20 in spec.MISSES and spec.MISSES[20].debt == 17
...
def test_debt_26_the_centre_is_the_missing_mass_loss_not_the_fitted_constant(basic):
    ...
    # the peak 1.39 -> 0.70, super-solar out to 1.9 kpc rather than 2.66 (D124). (The pre-D170 simple
    # model's windless centre read 0.25 and never reached +0.5.)
...
# --- debt #28: the flattening, measured ------------------------------------------

GRADS = ("metallicity_gradient_old", "metallicity_gradient_young")


def test_debt_28_migration_flattens_the_old_population_from_a_steeper_start(model):
    ...
    old0, young0 = (-0.254, -0.060)  # the advanced values; the simple model's (-0.228, -0.017) went at D170
    ...
    # young/old ratio came down with it, 2.5 (3.5 until S18).
...
def test_debt_42_row_6_is_at_the_edge_of_its_window(basic):
    ...
    # disc's observed dispersion net of the secular heating (88.8, D128), which leaves row 6 at 356, the heated
...
def test_debt_45_the_infall_scale_ratio_trades_the_structure_rows_against_the_gas_rows(model):
    ...
    # Upstream is shared physics: rows 1-4 and 20 read the same whatever the chemistry.
...
def test_the_register_carries_the_s10_findings():
    ...
    assert progress.debt_counts(text) == (47, 37)  # 45 / 37 at S34 (#98-#99, D183); 43 / 37 at S33; 27 / 35 at S22; S35 opened #100-#101 (D184)
    for item in (
...
def test_debt_43_the_solar_calibration_and_its_lever(basic):
    ...
    # platform's floating point: read -0.05949 on S22's container against S21b's -0.060 on its
    # own. +/-0.002 covers four times that spread and still halves S18's step, which is the size
    # this pin exists to see.
...
def test_debt_26_the_iron_at_the_centre_reaches_the_planets(basic):
    ...
    # S18 halved the centre's iron (debt #26: 1.39 -> 0.70 dex), so the occurrence inside a kiloparsec fell
    # 0.36 -> 0.25 and the metal-rich share of the catalogue 0.01 -> 0.005.
    assert 0.2 < inner(out) < 0.3
    # S19 re-pinned the catalogue number, and the direction is the finding. With the catalogue
    # migrating, the stars at any radius are drawn from the whole disc rather than from the ring
    # they sit in, so the local age-metallicity relation stops dominating: the metal-rich share
    # 0.005 -> 0.0031. The centre's iron still reaches the planets -- that is the occurrence
    # assertion above, unmoved -- but the *sample* is a weaker instrument for it than it was,
    # because a metal-rich star born at 1 kpc is now spread over the disc instead of counted
    # where it formed (D126). The windless simple model's contrast (1.25x) went with it at D170.
    assert 0.002 < rich(out) < 0.005  # 0.003-0.008 until S19
...
def test_debt_49s_prediction_ran_the_first_infall_on_the_halos_dynamical_time_and_failed(basic, monkeypatch):
    ...
    from constants the model has (1 Gyr, the two-infall framework's, reads the same). On the
    simple model's merger split (gone at D170) row 5 landed only where row 11 was far out, and
...
def test_debt_27_the_halos_own_accretion_history_as_the_first_infall_makes_no_thick_disc(basic, monkeypatch):
    ...
    almost nothing has arrived by the merger (the simple model's merger-split thick disc read 1.5e9,
...
def test_s21b_four_published_scalars_reach_no_surface_of_the_viewer(model):
    ...
    assert sorted(lost) == [
        ...
        "hii_luminosity_function_slope", "mean_planets_per_star", "planet_count_sample",
    ], lost  # the four cloud scalars joined at S32 (D181), the /api/clouds header carrying them; the
    # cluster census's two at S33, the /api/clusters header carrying them; the nebular stage's five at S35
    ...
    assert len(reachable) == len(fields) - len(lost) and len(lost) == 15  # 4 until S32's cloud scalars (D181), 8 until S33's two, 10 until S35's five
...
def test_s21b_s20s_numbers_reach_the_viewer(basic):
    ...
    field is still a galaxy scalar the viewer prints, and the radial spread is still a picture.
    """
    from galaxy.api.service import Service
